...
STRUCTV7_REMOTE_ROOT = "/path/to/user/m4oe/StructV7.5.4"
DEFAULT_CHSIMS_ROOT = "/path/to/user/datasets/MER-unibench/sims-process"
DEFAULT_CHSIMS_EMBEDDING_CACHE_ROOT = "/path/to/user/m4oe/embedding_cache/chsims_faceonly_nf4_vctx02_fp16_20260518"
...
def _upsert_total_table_rows(args, checkpoint_summary):
    ...
    common_hparams = {
        ...
        "plateau_monitor": getattr(args, "plateau_monitor", "val_loss"),
        "plateau_relative_min_factor": getattr(args, "plateau_relative_min_factor", -1.0),
...
def _step_lr_scheduler(scheduler, args, val_loss, val_mae):
    if scheduler is None:
        return
    if args.scheduler == "plateau":
        scheduler.step(val_mae if getattr(args, "plateau_monitor", "val_loss") == "val_mae" else val_loss)
    else:
        scheduler.step()
...
def _build_lr_scheduler(optimizer, args):
    if args.scheduler == "plateau":
        ...
        min_lr = ([float(group["lr"]) * relative_floor for group in optimizer.param_groups]
                  if relative_floor >= 0.0 else args.min_lr)
        return optim.lr_scheduler.ReduceLROnPlateau(
            ...
            min_lr=min_lr,
            eps=0.0 if relative_floor >= 0.0 else 1e-8,
        )
...
def train(args):
    ...
    default_tokenizer_path = "/path/to/user/models/AI-ModelScope_roberta-base"
    ...
    import study_gradprobe
    param_groups = _build_optimizer_param_groups(model, contrast_head, args)
    param_groups = study_controls.configure(model, param_groups, args)
    if os.environ.get("STUDY_DIAG"):
        study_gradprobe.begin(model,args)
    print(
        "Optimizer groups: "
        + ", ".join(
    ...
    if writer is not None:
        ...
        writer.add_text("config/plateau_monitor", str(args.plateau_monitor), 0)
        writer.add_text("config/plateau_relative_min_factor", str(args.plateau_relative_min_factor), 0)
    ...
    for epoch in range(args.epochs):
        ...
        if freeze_backbone:
            ...
            model.resnet.eval()
        study_controls.enforce_epoch(model)
        if os.environ.get("STUDY_DIAG"):
            study_gradprobe.enforce_model_mode(model)
        total_loss = 0
        ...
        for step_idx, batch in enumerate(loop):
            ...
            if not torch.isfinite(loss_raw):
                ...
                continue
            if epoch == 0 and step_idx == 0 and not os.environ.get("STUDY_DIAG"):
                study_controls.component_probe(model, {"reg": loss_reg, "cls_weighted": args.cls7_loss_weight * loss_cls7, "gate_weighted": args.tcif_transition_gate_loss_weight * loss_tcif_transition_gate}, args)
            loss = loss_raw / max(1, args.grad_accum_steps)

            if os.environ.get("STUDY_DIAG"):
                study_gradprobe.capture(model, batch, model_out, loss_reg, loss_cls7, loss_tcif_transition_gate, loss_raw, epoch, step_idx, args)
                continue
            loss.backward()
            ...
            if do_step:
                if args.clip_grad_norm > 0:
                    ...
                    study_controls.log_gradient(epoch, step_idx, grad_norm, args)
        ...
        study_controls.save_validation(model, args, epoch+1, val_metrics, test_metrics, _update_best_checkpoints, _write_checkpoint_selection_summary)
        ...
        _step_lr_scheduler(scheduler, args, val_loss, val_mae)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tokenizer_path", type=str, default="/path/to/user/models/AI-ModelScope_roberta-base")
    parser.add_argument(
        "--bert_backbone_path",
        type=str,
        default="/path/to/user/models/AI-ModelScope_roberta-base",
    ...
    parser.add_argument("--vit_backbone_path", type=str, default="/path/to/user/models/vit-base-patch16-224-in21k")
    ...
    parser.add_argument("--reg_loss_type", type=str, default="smooth_l1", choices=["mse", "smooth_l1", "l1"])

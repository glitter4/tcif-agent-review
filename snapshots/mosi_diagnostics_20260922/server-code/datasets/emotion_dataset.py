...
def _attach_temporal_context(dataset, idx, item):
    if dataset.temporal_context_radius <= 0:
        return item

    context_rows = dataset.temporal_context_index[idx]
    if not context_rows:
        raise RuntimeError(
            "temporal_context_radius is positive but no context slots were built"
        )
    context_items = [
        dataset._get_single_item(row["index"])
...
class MultimodalEmotionDataset(Dataset):
    def __init__(
        ...
        split='train',
        transform=None,
        tokenizer_path="/path/to/user/models/google-bert_bert-base-uncased",
        max_length=128,
...
class CMUMOSEIProcessDataset(Dataset):
    def __init__(
        self,
        root="/path/to/user/datasets/MER-unibench/cmumosei-process",
        split="train",
        transform=None,
        tokenizer_path="/path/to/user/models/google-bert_bert-base-uncased",
        max_length=128,
...
class CHSIMSProcessDataset(Dataset):
    """CH-SIMS process-format loader with the same cached-token contract as MOSEI.

    CH-SIMS differs from CMU-MOSEI mainly in its [-1, 1] label range and audio
    directory layout.  Keeping a dedicated loader prevents dataset-specific path
    and transcription conventions from leaking into the model architecture.
    """

    def __init__(
        self,
        root="/path/to/user/datasets/MER-unibench/sims-process",
        split="train",
        transform=None,
        tokenizer_path="/path/to/user/models/google-bert_bert-base-uncased",
        max_length=128,
        ...
        self.temporal_context_mode = str(temporal_context_mode).lower()
        self.temporal_context_seed = int(temporal_context_seed)
        if self.valence_scale <= 0:
            raise ValueError("valence_scale must be positive")
        ...
        with open(trans_path, "r", encoding="utf-8") as f:
            reader = csv.reader(f)
            for row in reader:
                if not row or row[0] in {"id", "name", "video_id"}:
                    continue
                if len(row) >= 3:
                    self.text_by_id[row[0]] = row[2] or row[1]
                elif len(row) == 2:
                    self.text_by_id[row[0]] = row[1]
                else:
                    self.text_by_id[row[0]] = ""
    ...
    def _load_openface_frames(self, sample_id: str):
        ...
        total = int(arr.shape[0])
        if total <= 0:
            raise ValueError(f"Empty OpenFace face track for sample '{sample_id}': {npy_path}")
        frames = [arr[i] for i in _uniform_frame_indices(total, self.num_frames)]
        if not frames:
            raise ValueError(
                f"Could not sample any OpenFace frames for sample '{sample_id}' from {npy_path}"
            )
    ...
    def _load_openface_frame_triplets(self, sample_id: str):
        ...
        if total <= 0:
            raise ValueError(f"Empty OpenFace face track for sample '{sample_id}': {npy_path}")
        triplets = [
            [arr[left_idx], arr[center_idx], arr[right_idx]]
            for left_idx, center_idx, right_idx in _context_frame_triplets(
                total, self.num_frames, self.vit_context_ratio
            )
        ]
        if not triplets:
            raise ValueError(
        ...
        while len(triplets) < self.num_frames:
            triplets.append(triplets[-1])
        return triplets[: self.num_frames]

    def _load_image_tensor(self, sample_id: str):
        if self.vit_context_ratio > 0.0:
            return _stack_frame_triplets(
                self._load_openface_frame_triplets(sample_id),
                self.transform,
                self.num_frames,
                fallback_size=112,
            )
        return _stack_frames(
            self._load_openface_frames(sample_id),
            self.transform,
            self.num_frames,
            fallback_size=112,
        )
    ...
    def _get_single_item(self, idx):
        ...
        temporal_metadata = self.temporal_metadata_by_id[sample_id]
        input_ids, attention_mask = self.encoded_text[idx]
        val = float(self.labels[sample_id]["val"]) * self.valence_scale
        intensity = _valence_to_intensity(val)
        polarity = _valence_to_polarity(val)
        ...
        if cached is not None:
            item = {
                ...
                "temporal_pos": torch.tensor(
                    temporal_metadata["temporal_pos"], dtype=torch.float32
                ),
                "temporal_group_id": torch.tensor(
                    temporal_metadata["temporal_group_id"], dtype=torch.long
                ),
                "audio_values": torch.empty(0, dtype=torch.float32),
                "audio_attention_mask": torch.empty(0, dtype=torch.long),
            }
        ...
        image = self._load_image_tensor(sample_id)
        waveform, sr = _load_audio_from_path(
            os.path.join(self.audio_dir, f"{sample_id}.wav")
        )
        waveform, audio_mask, _ = preprocess_audio(
            waveform,
            orig_sr=sr,
    ...
    def __getitem__(self, idx):
        return _attach_temporal_context(self, idx, self._get_single_item(idx))

class CMUMOSIProcessDataset(Dataset):
    def __init__(
        self,
        root="/path/to/user/datasets/MER-unibench/cmumosi-process",
        split="train",
        transform=None,
        tokenizer_path="/path/to/user/models/google-bert_bert-base-uncased",
        max_length=128,

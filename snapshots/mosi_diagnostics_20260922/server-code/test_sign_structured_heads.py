...
from models.models_emotion import SharedSpecificMoELayer


def test_chsims_five_bin_centers_boundaries_and_loss():
    centers = get_cls7_centers(num_classes=5, label_min=-1.0, label_max=1.0)
    assert torch.allclose(
        centers, torch.tensor([-0.9, -0.4, 0.0, 0.4, 0.9])
    )
    values = torch.tensor([-1.0, -0.7, -0.6999, -0.1, -0.0999, 0.1, 0.1001, 0.7, 0.7001, 1.0])
    expected = torch.tensor([0, 0, 1, 1, 2, 2, 3, 3, 4, 4])
    assert torch.equal(continuous_to_cls7_hard(values, centers=centers), expected)
    logits = torch.randn(values.numel(), 5, requires_grad=True)
    loss, target = _compute_cls7_loss(
        logits,
        values,
        "hard_ce",
        0.5,
...
def test_expert_mlp_ratio_and_argmax_readout():
    ...
    assert layer.expert_mlp_ratio == 2.0
    assert all(expert.fc1.out_features == 16 for expert in layer.experts)

    centers = get_cls7_centers(num_classes=5, label_min=-1.0, label_max=1.0)
    logits = torch.tensor([[0.0, 0.0, 0.0, 5.0, 1.0]])
    decoded = compute_final_prediction(
        torch.tensor([-0.5]),
        logits,
        eta=1.0,
        centers=centers,
        classification_readout="argmax",
    )
    assert torch.allclose(decoded, torch.tensor([0.4]))
...
def main():
    test_chsims_five_bin_centers_boundaries_and_loss()
    test_expert_mlp_ratio_and_argmax_readout()

...
def test_tcif_context_head_and_auxiliary_loss_support_five_bins():
    torch.manual_seed(124)
    model = build_tiny_model(
        "signed_reg_cls7",
        signed_class_count=5,
        label_min=-1.0,
        label_max=1.0,
        enable_tcif=True,
        tcif_latent_dim=4,
    ...
    model.eval()
    batch = tiny_batch()
    batch["raw_valence"] = torch.tensor([-0.8, 0.3], dtype=torch.float32)
    with torch.no_grad():
        output = model(
            batch["image"],
            batch["input_ids"],
    ...
    payload = output["extras"]["tcif"]
    assert tuple(payload["context_cls7_logits"].shape) == (2, 5)
    ...
    total, regression, ordinal, valid_count = _compute_tcif_context_aux_loss(
        output,
        batch["raw_valence"],
        args,
    )
    ...
    assert torch.isfinite(regression)
    assert torch.isfinite(ordinal)

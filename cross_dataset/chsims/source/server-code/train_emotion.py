from pathstudy_support import configure_stage, stage_groups, stage_mode
import os
import sys
import argparse
import time
...
from chsims_followup_support import guarded_metrics, update_guarded, record_temporal, flush_temporal, record_lr
...
from datasets.emotion_dataset import CHSIMSProcessDataset, CMUMOSEIProcessDataset, CMUMOSIProcessDataset, MultimodalEmotionDataset, _valence_to_polarity, _valence_to_7class, _valence_to_binary
...
STRUCTV7_REMOTE_ROOT = "/path/to/outputs/StructV7.5.4"
DEFAULT_CHSIMS_ROOT = "/path/to/datasets/sims-process"
DEFAULT_CHSIMS_EMBEDDING_CACHE_ROOT = "/path/to/embedding_cache/chsims_faceonly_nf4_vctx02_fp16_20260518"
...
def _signed_centers(args, device=None, dtype=torch.float32):
    return get_cls7_centers(
        device=device,
        dtype=dtype,
        num_classes=int(getattr(args, "signed_class_count", 7)),
        label_min=float(getattr(args, "label_min", -3.0)),
        label_max=float(getattr(args, "label_max", 3.0)),
    )


def _signed_metric_name(args):
    return f"acc{int(getattr(args, 'signed_class_count', 7))}"
...
def _collect_train_raw_valences(train_dataset):
    values = []
    if isinstance(train_dataset, (CMUMOSEIProcessDataset, CMUMOSIProcessDataset, CHSIMSProcessDataset)):
...
def compute_center_regression_loss(prediction, target, args):
    weight = float(getattr(args, "center_reg_loss_weight", 1.0))
    if not math.isfinite(weight) or weight < 0:
        raise ValueError("center_reg_loss_weight must be finite and nonnegative")
    if getattr(args, "center_reg_loss_type", "inherit") == "l1":
        return weight * F.l1_loss(prediction, target, reduction="mean")
    return weight * compute_regression_loss(prediction, target, args.reg_loss_type)
...
def _compute_cls7_loss(
    ...
    centers = (
        get_cls7_centers(device=logits.device, dtype=logits.dtype)
        if centers is None
        else centers.to(device=logits.device, dtype=logits.dtype)
    )
    if centers.numel() != logits.shape[-1]:
        raise ValueError(
            f"signed-center/logit mismatch: centers={centers.numel()} logits={logits.shape[-1]}"
        )
    sample_weights = _cls7_sample_weights(raw_valence, low_abs_threshold, low_abs_weight)
    if loss_type == "hard_ce":
        target = continuous_to_cls7_hard(raw_valence, centers=centers)
        ce_weights = None
    ...
    if loss_type == "soft_ce":
        ...
        hard_target = continuous_to_cls7_hard(raw_valence, centers=centers)
        if class_weights is not None:
...
def _compute_tcif_context_aux_loss(model_out, raw_valence, args):
    ...
    context_cls7_target = continuous_to_cls7_hard(
        raw_valence[has_context],
        centers=_signed_centers(
            args,
...
def _compute_sign_marginal_loss(cls7_logits, raw_valence, args):
    ...
    if class_count % 2 != 1:
        raise ValueError("sign-marginal loss requires an odd signed class count")
    neutral_index = class_count // 2
    p_neg = probs[:, :neutral_index].sum(dim=-1)
    p_pos = probs[:, neutral_index + 1:].sum(dim=-1)
...
def _compute_temporal_contrast_loss(model_out, batch, args):
    ...
    record_temporal(batch, args, stats)
    return loss, stats


def _maybe_clamp_eval(y, clamp_eval: bool, label_min=-3.0, label_max=3.0):
    return y.clamp(float(label_min), float(label_max)) if clamp_eval else y
...
def _signed_prediction_tensors(outputs, args):
    ...
    centers = _signed_centers(args, device=cls7_logits.device, dtype=cls7_logits.dtype)
    y_reg_eval = _maybe_clamp_eval(
        y_reg,
        args.clamp_regression_eval,
        label_min=getattr(args, "label_min", -3.0),
        label_max=getattr(args, "label_max", 3.0),
    )
...
def score_metrics_from_lists(scores, raw_valences, centers=None):
    rmse, mae, pearson, spearman = regression_metrics(scores, raw_valences)
    centers = get_cls7_centers() if centers is None else torch.as_tensor(centers)
    pred_7 = continuous_to_cls7_hard(
        torch.tensor(scores, dtype=torch.float32), centers=centers
    ).tolist()
    gt_7 = continuous_to_cls7_hard(
        torch.tensor(raw_valences, dtype=torch.float32), centers=centers
    ).tolist()
    class_count = int(centers.numel())
    acc7, macro_f1_7, _, _ = metrics_from_lists(
        gt_7, pred_7, class_count
    )
...
def summarize_signed_metrics(
    ...
    centers = get_cls7_centers() if centers is None else torch.as_tensor(centers)
    class_count = int(centers.numel())
    cls7_acc, cls7_macro_f1, cls7_precision, cls7_recall = metrics_from_lists(
        cls7_labels, cls7_preds, class_count
    )
    return {
        "reg": score_metrics_from_lists(reg_scores, raw_valences, centers=centers),
        "cls_expected": score_metrics_from_lists(
            cls_expected_scores, raw_valences, centers=centers
        ),
        "final": score_metrics_from_lists(final_scores, raw_valences, centers=centers),
        "cls7_acc": cls7_acc,
        "cls7_macro_f1": cls7_macro_f1,
        "cls7_precision": cls7_precision,
        "cls7_recall": cls7_recall,
        "signed_class_count": class_count,
    }
...
def _upsert_total_table_rows(args, checkpoint_summary):
    ...
    common_hparams = {
        ...
        "expert_mlp_ratio": args.expert_mlp_ratio,
...
def _build_lr_scheduler(optimizer, args):
    if args.scheduler == "plateau":
        relative_floor = float(getattr(args, "plateau_relative_min_factor", -1.0))
        if relative_floor != -1.0 and not 0.0 <= relative_floor <= 1.0:
            raise ValueError("plateau_relative_min_factor must be -1 or in [0, 1]")
        min_lr = [group["lr"] * relative_floor for group in optimizer.param_groups] if relative_floor >= 0 else args.min_lr
        return optim.lr_scheduler.ReduceLROnPlateau(
            ...
            min_lr=min_lr,
            eps=0.0 if relative_floor >= 0 else 1e-8,
        )
...
def _compute_cls7_class_weights(
    train_dataset, mode: str, max_weight: float = 0.0, centers=None
):
    centers = get_cls7_centers() if centers is None else torch.as_tensor(centers)
    class_count = int(centers.numel())
    mode = str(mode or "none")
    if mode == "none":
        return None, [0 for _ in range(class_count)]
    values = []
    if isinstance(train_dataset, (CMUMOSEIProcessDataset, CMUMOSIProcessDataset, CHSIMSProcessDataset)):
        for sample_id in train_dataset.ids:
            values.append(float(train_dataset.labels[sample_id]["val"]))
    ...
    if values:
        labels = continuous_to_cls7_hard(
            torch.tensor(values, dtype=torch.float32), centers=centers
        )
...
def _collect_raw_valence_summary(dataset):
    values = []
    if isinstance(dataset, (CMUMOSEIProcessDataset, CMUMOSIProcessDataset, CHSIMSProcessDataset)):
...
def train(args):
    ...
    if args.dataset == "cmumosei":
        ...
        chsims_common = {
            ...
            "vit_context_ratio": args.vit_context_ratio,
            "embedding_cache_root": args.embedding_cache_root,
            "valence_scale": args.chsims_valence_scale,
            "temporal_context_radius": (args.tcif_context_radius if args.enable_tcif else 0),
            "temporal_context_mode": args.tcif_context_mode,
            "temporal_context_seed": args.seed,
        }
        train_dataset = CHSIMSProcessDataset(split="train", **chsims_common)
        val_dataset = CHSIMSProcessDataset(split="val", **chsims_common)
        test_dataset = CHSIMSProcessDataset(split="test", **chsims_common)
    else:
    ...
    if raw_summary is not None:
        ...
        if args.dataset != "chsims" and -1.05 <= raw_summary["min"] and raw_summary["max"] <= 1.05:
    ...
    model = EmotionM4OE(
        ...
        output_head_mode=args.output_head_mode,
        final_pred_eta=args.final_pred_eta,
        signed_class_count=args.signed_class_count,
    ...
    configure_stage(model, args)
    param_groups = _build_optimizer_param_groups(model, contrast_head, args)
    param_groups = stage_groups(model, param_groups, args)
    print(
        "Optimizer groups: "
        + ", ".join(
    ...
    if writer is not None:
        ...
        writer.add_text("config/expert_mlp_ratio", str(args.expert_mlp_ratio), 0)
    ...
    for epoch in range(args.epochs):
        ...
        for step_idx, batch in enumerate(loop):
            ...
            if args.output_head_mode == "legacy":
                ...
                loss_reg = compute_center_regression_loss(y_reg, raw_valence, args)
                loss_cls7, cls7_target = _compute_cls7_loss(
                    ...
                    centers=_signed_centers(
                        args, device=cls7_logits.device, dtype=cls7_logits.dtype
                    ),
                )
            ...
            if args.stage2_mode == "head_only":
                loss_raw = args.cls7_loss_weight * loss_cls7
        ...
        if args.enable_guarded_checkpoint:
            update_guarded(model, args, epoch + 1,
                           val_loss_parts["metrics"]["guarded_argmax"],
                           test_loss_parts["metrics"]["guarded_argmax"],
                           best_checkpoint_records, _write_checkpoint_hparams)
        ...
        lr_before = {group.get("name", str(i)): group["lr"] for i, group in enumerate(optimizer.param_groups)}
        if scheduler is not None:
            if args.scheduler == "plateau":
                scheduler.step(val_mae if args.plateau_monitor == "val_mae" else val_loss)
            else:
                scheduler.step()
        record_lr(args, optimizer, epoch + 1, lr_before, val_mae)
...
def validate_signed_reg_cls7(model, loader, device, eff_alpha, args, contrast_head=None):
    ...
    if getattr(args, "enable_guarded_checkpoint", False):
        metrics["guarded_argmax"] = guarded_metrics(reg_scores, cls7_preds_all, raw_valences, _signed_centers(args))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="csv", choices=["csv", "cmumosei", "cmumosi", "chsims"])
    ...
    parser.add_argument("--chsims_valence_scale", type=float, default=1.0)
    ...
    parser.add_argument("--expert_mlp_ratio", type=float, default=1.0)
    ...
    parser.add_argument("--plateau_monitor", choices=["val_loss", "val_mae"], default="val_loss")
    parser.add_argument("--plateau_relative_min_factor", type=float, default=-1.0)
    ...
    parser.add_argument("--signed_class_count", type=int, default=7)
    parser.add_argument("--label_min", type=float, default=-3.0)
    parser.add_argument("--label_max", type=float, default=3.0)
    parser.add_argument("--reg_loss_type", type=str, default="smooth_l1", choices=["mse", "smooth_l1"])
    parser.add_argument("--center_reg_loss_type", choices=["inherit", "l1"], default="inherit")
    parser.add_argument("--center_reg_loss_weight", type=float, default=1.0)
    parser.add_argument("--enable_guarded_checkpoint", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--tcif_detach_context_features", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--stage2_mode", choices=["none", "head_only", "full_continue"], default="none")
    parser.add_argument("--init_checkpoint", default="")
    parser.add_argument("--enable_dev_aligned_checkpoint", action=argparse.BooleanOptionalAction, default=False)
    ...
    if args.dataset == "chsims":
        if args.dataset_root == "/path/to/datasets/cmumosei-process-complete-20260408":
            args.dataset_root = DEFAULT_CHSIMS_ROOT
        if not args.embedding_cache_root:
            args.embedding_cache_root = DEFAULT_CHSIMS_EMBEDDING_CACHE_ROOT
        if (
            args.signed_class_count == 7
            and args.label_min == -3.0
            and args.label_max == 3.0
        ):
            args.signed_class_count = 5
            args.label_min = -1.0
            args.label_max = 1.0
    ...
    if args.signed_class_count <= 1 or args.signed_class_count % 2 != 1:
        raise ValueError("--signed_class_count must be an odd integer greater than 1")
    if args.expert_mlp_ratio <= 0:
        raise ValueError("--expert_mlp_ratio must be positive")
    if args.label_max <= args.label_min:
        raise ValueError("--label_max must be greater than --label_min")
    if args.chsims_valence_scale <= 0:
        raise ValueError("--chsims_valence_scale must be positive")
    if args.signed_class_count != 7 and args.cls7_head_type != "flat":
        raise ValueError("non-7-bin datasets currently require --cls7_head_type flat")
    if args.dataset == "chsims" and args.output_head_mode != "signed_reg_cls7":
        raise ValueError("CH-SIMS requires --output_head_mode signed_reg_cls7")
    if args.signed_class_count != 7 and (
        args.signed_neutral_band_weight > 0
        or args.reg_cls_mag_consistency_weight > 0
        or args.neutral_positive_gate_threshold is not None
    ):
        raise ValueError(
            "signed-neutral-band, magnitude-consistency and neutral-positive-gate "
            "features currently use MOSEI/MOSI 7-bin boundaries and are disabled "
            "for non-7-bin datasets"
        )
    ...
    if args.enable_tcif:
        if args.dataset not in {"cmumosei", "cmumosi", "chsims"}:
            raise ValueError(
                "TCIF requires a process-format temporal dataset: "

...
from head_utils import compute_final_prediction, get_cls7_centers
...
def discover_models(checkpoints_root: str):
    targets = {"best_mae_model.pth", "best_acc7_model.pth", "best_model.pth"}
    found = []
...
def evaluate_split(
        ...
        for batch in loader:
            ...
            if getattr(model, "output_head_mode", "legacy") == "legacy":
                ...
                centers = get_cls7_centers(
                    device=cls7_logits.device, dtype=cls7_logits.dtype
                )
                y_reg_eval = y_reg.clamp(-3.0, 3.0) if clamp_regression_eval else y_reg
                ...
                if isinstance(tcif_payload, dict):
                    ...
                    if clamp_regression_eval:
                        context_y_reg = context_y_reg.clamp(-3.0, 3.0)
    ...
    for pv, rv in zip(pred_vals, raw_vals):
        pb = _valence_to_binary(pv)
        gb = _valence_to_binary(rv)
        p7 = _valence_to_7class(pv)
        g7 = _valence_to_7class(rv)
        pred_bin.append(pb)
        gt_bin.append(gb)
        pred_7.append(p7)
        gt_7.append(g7)
    acc2 = accuracy_from_lists(gt_bin, pred_bin)
    acc7, f1_7, _, _ = metrics_from_lists(gt_7, pred_7, 7)
    ...
    if tcif_context_vals:
        ...
        context_pred_7 = [_valence_to_7class(v) for v in context_preds]
        context_gt_7 = [_valence_to_7class(v) for v in context_targets]
...
def main():
    ...
    parser.add_argument(
        "--checkpoints_root",
        type=str,
        default="/path/to/project/m4oe/StructV7.0/train/checkpoints",
    )
    parser.add_argument(
        "--results_root",
        type=str,
        default="/path/to/project/m4oe/StructV7.0/eval_all_mosei_maefixed",
    )
    parser.add_argument(
        "--default_dataset",
        type=str,
        default="cmumosei",
        choices=["cmumosei", "cmumosi"],
    )
    parser.add_argument(
    ...
    for item in discovered:
        ...
        if dataset not in {"cmumosei", "cmumosi"}:
        ...
        tcif_ablation = str(hparams.get("tcif_ablation", "full"))
        ...
        if enable_tcif and dataset != "cmumosei":
            raise ValueError("TCIF stage-1 evaluator currently supports CMU-MOSEI only")

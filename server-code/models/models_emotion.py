...
from models.tcif import TCIF_OUTPUT_MODES, build_task_filters
...
class SharedSpecificMoELayer(nn.Module):
    def __init__(
        ...
        self.experts = nn.ModuleList([
            Mlp(in_features=self.dim, out_features=self.dim, drop=drop)
...
class EmotionM4OE(nn.Module):
    def __init__(self, 
                 ...
                 vision_backbone_path="/path/to/models/resnet-18",
                 vit_model_path="/path/to/models/vit-base-patch16-224-in21k",
                 bert_model_path="/path/to/models/google-bert_bert-base-uncased",
                 hubert_model_path="/path/to/models/hubert-base-ls960",
        ...
        self.register_buffer("cls7_centers", get_cls7_centers(), persistent=False)
        ...
        self.tcif_ablation = validate_ablation(tcif_ablation)
        if self.tcif_ablation != "full" and not self.enable_tcif:
            raise ValueError("TCIF ablation requires enable_tcif=True")
        ...
        if self.output_head_mode == "legacy":
            ...
            self.head_cls7 = nn.Linear(self.head_input_dim, 7) if self.cls7_head_type in {"flat", "hybrid_cumulative"} else None
        ...
        if self.enable_tcif:
            self.tcif_regression, self.tcif_ordinal = build_task_filters(
                ablation=self.tcif_ablation,
            ...
            self.tcif_context_cls7_head = nn.Linear(self.tcif_latent_dim, 7)
        else:
    ...
    def _load_backbone(cls, model_path: str, local_files_only: bool, fallback_model: nn.Module) -> nn.Module:
        ...
        try:
            ...
            pass
        try:
            return AutoModel.from_pretrained(model_path, local_files_only=local_files_only)
